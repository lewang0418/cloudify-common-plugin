...
class _Storage(object):
    ...
    def _add_lock(self, node_instance_id):
        self._locks[node_instance_id] = threading.RLock()

    def _delete_lock(self, node_instance_id):
        if not self._locks.get(node_instance_id, None):
            del self._locks[node_instance_id]
...
class FileStorage(_Storage):
    ...
    def add_node_to_data(self, node):
        node_instance = NodeInstance(node)
        self.plan['node_instances'].append(node_instance)
        self.data['plan'] = self.plan

        for node in self.data['nodes']:
            if node['id'] == node_instance.node_id:
                node['deploy_number_of_instances'] += 1
                node['number_of_instances'] += 1
                node['instances']['deploy'] += 1

        data_path = os.path.join(self._storage_dir, 'data')
        with open(data_path, 'w') as f:
            f.write(json.dumps(self.data))

    def delete_node_from_data(self, node):
        node_instance = NodeInstance(node)

        for instance in self.plan['node_instances']:
            if instance['id'] == node_instance.id:
                self.plan['node_instances'].remove(instance)
        self.data['plan'] = self.plan

        for node in self.data['nodes']:
            if node['id'] == node_instance.node_id:
                if node['deploy_number_of_instances'] > 0:
                    node['deploy_number_of_instances'] -= 1
                if node['number_of_instances'] > 0:
                    node['number_of_instances'] -= 1
                if node['instances']['deploy'] > 0:
                    node['instances']['deploy'] -= 1
    ...
    def add_node_instance(self, instance, lock=True):
        instance_lock = None
        node_instance = NodeInstance(instance)
        if not self._locks.get(node_instance.id, None):
            self._add_lock(node_instance.id)
    ...
    def delete_node_instance(self, instance, lock=True):
        instance_lock = None
        node_instance = NodeInstance(instance)
        ...
        if lock:
            ...
            instance_lock.acquire()
        try:
            if os.path.exists(self._instance_path(node_instance.id)):
                os.remove(self._instance_path(node_instance.id))
            else:
                raise RuntimeError('Instance {0} does not exist'
                                   .format(node_instance_id))
        finally:
            if lock and instance_lock:
                instance_lock.release()
                self._delete_lock(node_instance.id)
