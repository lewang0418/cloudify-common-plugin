...
#        raise NotImplementedError('Implemented by subclasses')
        ...
        modification = DeploymentModification(
            id=modification_id,
            created_at=now,
            ended_at=None,
            status=DeploymentModification.STARTED,
    ...
    def finish_deployment_modification(self, modification):
        if modification.status in DeploymentModification.END_STATES:
            raise 'Cannot finish deployment modification: {0}. It is already in {1} status.'. \
                    format(modification.id, modification.status)
        ...
        return DeploymentModification(
            id=modification.id,
            status=DeploymentModification.FINISHED,
            ended_at=None,
            created_at=None,
            deployment_id=None,
            modified_nodes=None,
    ...
    def rollback_deployment_modification(self, modification):
        ...
        modification.node_instances['before_rollback'] = node_instances

        for instance in node_instances:
            storage.delete_node_instance(instance)
            storage.delete_node_from_data(instance)
        for instance in modification.node_instances['before_modification']:
            storage.add_node_instance(instance)
            storage.add_node_to_data(instance)

        now = str(datetime.now())
        return DeploymentModification(
            id=modification.id,
            status=DeploymentModification.ROLLEDBACK,
            ended_at=now,
            created_at=None,
            deployment_id=None,
            modified_nodes=None,
